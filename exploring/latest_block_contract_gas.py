...
def analyze_latest_block_gas():
    ...
    if not w3.eth.is_syncing():
        ...
        if block_trx_data:
            ...
            """
            Attribute simple transaction gas
            """
            ...
            print('Contract Gas Used - {}'.format(dapp_gas_used))
            print(dapp_grouped)
            ...
            print('Contract Block Gas Used - {}'.format(dapp_grouped['gasUsed'].sum()))
            print('Simple Transaction Gas Used Percent - {:.2%}'.format((simple_trx_gas_used / block.get('gasUsed'))))
            print('Anon Bot Gas Used Percent - {:.2%}'.format((total_anon_bots_usage / block.get('gasUsed'))))
            print('Contract Gas Used Percent - {:.2%}'.format((dapp_gas_used / block.get('gasUsed'))))
            print('Attributed Gas Used Percent - {:.2%}'.format(
                ((dapp_gas_used + simple_trx_gas_used + total_anon_bots_usage) / block.get('gasUsed'))))
        else:
            print('Block Number {} has no transactions'.format(block.number))
    else:
