...
export const syncBlock = async (blockNumber: number, skipParentCheck = false): Promise<void> => {
  ...
  if (!skipParentCheck && !isParentKnown && !(blockNumber === Blocks.londonHardForkBlockNumber)) {
    Log.warn(
  ...
  await Blocks.storeBlock(block, txrs, ethPrice.ethusd, skipParentCheck);
};
...
export const syncBlocksStartingFrom = (upToIncluding: number, startBlock: number): T.Task<void> =>
  pipe(
    T.Do,
    // If a rollback happened while we were offline, rollback to the last valid block.
    T.apS("_rollbackToLastValid", rollbackToLastValidBlock),
    ...
    T.chain(({ syncedBlockHeight }) =>
      syncedBlockHeight === upToIncluding
        ...
        : pipe(
            ...
            T.apS(
              "blocksToSync",
              T.of(NEA.range(Math.max(startBlock, syncedBlockHeight) + 1, upToIncluding)),
            ),
            T.chain(({ blocksToSync }) => async () => {
              Log.debug(
                `blocks table sync ${
              ...
              for (const blockNumber of blocksToSync) {
                await syncBlock(blockNumber, true);
                blocksDone = blocksDone + 1;
                logEta();
              }
            }),
