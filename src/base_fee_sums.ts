...
const getTimeframeBaseFeeSum = (
  ...
  pipe(
    () => sql<BaseFeeSum[]>`
      ...
      FROM blocks
      WHERE mined_at >= now() - ${TimeFrames.intervalSqlMap[timeframe]}::interval
...
const getBaseFeeSum = (block: BlockDb): T.Task<BaseFeeSum> =>
  pipe(
    () => sql<BaseFeeSum[]>`
      SELECT
        SUM(base_fee_sum) AS eth,
...
export const calcBaseFeeSums = (block: BlockDb): T.Task<FeesBurnedT> =>
  pipe(
    TAlt.seqSParT({
